...
public class MouseWerewolf extends AbstractWerewolf {
	...
	public String talk() {
		//CO、パワープレイなどの設定
		//パワープレイは誰かに提案された場合のみ確率に基づいて行う

		//String[] arr_werewolfs = (String[]) werewolfs.toArray(new String[]{});

		//aliveList += 1;
		//ContentBuilder builder = new ComingoutContentBuilder(me,Role.WEREWOLF);
		//return new Content(builder).getText();

		return "人数:" + currentGameInfo.getAgentList().size() + " ,役職:" + currentGameInfo.getExistingRoles().size();
		//return "けもみみもふもふ";
		//return String.valueOf(talkcount);
		//return String.join(",", arr_werewolfs);
	}
